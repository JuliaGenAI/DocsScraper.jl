...
- index_name: Name of the index. Default: "index" symbol generated by gensym
"""
function generate_embeddings(chunks::Vector{SubString{String}};
        model_embedding::AbstractString = MODEL_EMBEDDING,
        embedding_dimension::Int = EMBEDDING_DIMENSION, embedding_bool::Bool = EMBEDDING_BOOL,
        index_name::AbstractString = "")
    ...
    full_embeddings = RT.get_embeddings(
        embedder, chunks; model = model_embedding, verbose = false, cost_tracker)
...
function save_embeddings(index_name::AbstractString, embedding_dimension::Int,
    ...
    tar_file = joinpath(target_path, "Index",
        "$(file_name).tar.gz")
    hdf5_file = joinpath(target_path, "Index",
        "$(file_name).hdf5")
    ...
    command = `tar -cvzf $tar_file -C $(dirname(hdf5_file)) $(basename(hdf5_file))`
    run(command)
    report_artifact(tar_file, target_path, index_name)
    return hdf5_file
end
